...
export default function SellerOrdersPage() {
  ...
  const { orders, updateOrderStatus } = useOrders(undefined, sellerId);
  ...
  const columns = useMemo<ColumnDef<OrderRow>[]>(
      ...
      },
      {
        ...
        header: ({ column }) => (
          <button
            ...
            type="button"
          >
            {t("customer")} <ArrowUpDown size={14} aria-hidden="true" />
          </button>
        ),
        cell: ({ row }) => (
          <div>
            <p className={styles.customerName}>{row.original.buyerName}</p>
            <p className={styles.customerEmail}>{row.original.buyerEmail}</p>
          </div>
        ),
      },
      {
        ...
        header: ({ column }) => (
          <button
            ...
            type="button"
          >
            {t("total")} <ArrowUpDown size={14} aria-hidden="true" />
          </button>
        ),
        cell: ({ getValue }) => (
    ...
    [t, statusLabels, openStatusModal],
  );
